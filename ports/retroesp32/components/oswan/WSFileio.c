...
/* WsCreateFromMemory maps ROM pages directly into the caller-owned buffer.
 * Keep that separate from WsCreate(), whose pages are individually malloc'd,
 * so cleanup never frees interior ROM pointers. */
static const BYTE *s_rom_data = NULL;
static BYTE *s_cart_ram = NULL;
...
void WsRelease(void)
{
    ...
    if (SaveName[0] != 0)
    {
        if ((fp = fopen(SaveName, "wb"))!= NULL)
        {
            for (i  =0; i < RAMBanks; i++)
            {
                if (!RAMMap || RAMMap[i] == MemDummy || RAMMap[i] == NULL)
                    continue;
                if (RAMSize<0x10000)
                {
    ...
    /* PAPP launches do not set SaveName, but cartridge RAM still needs to be
     * released when control returns to the launcher. */
    if (RAMMap)
    {
        for (i = 0; i < 0x100; ++i)
        {
            if (RAMMap[i] != MemDummy && RAMMap[i] != NULL)
                free(RAMMap[i]);
            RAMMap[i] = MemDummy;
        }
    }
    ...
    /* XIP ROM pages point into the PAPP-owned ROM buffer and must not be
     * freed.  File-backed WsCreate() pages are individually owned here. */
    for (i = 0; i < 0x100; ++i)
    {
        if (ROMMap && ROMMap[i] != MemDummy && ROMMap[i] != NULL &&
            s_rom_data == NULL)
            free(ROMMap[i]);
        if (ROMMap) ROMMap[i] = MemDummy;
    }
    s_rom_data = NULL;

    if (ROMMap) { free(ROMMap); ROMMap = NULL; }
    if (RAMMap) { free(RAMMap); RAMMap = NULL; }
    if (IO) { free(IO); IO = NULL; }
    if (IRAM) { free(IRAM); IRAM = NULL; }
    ROMBanks = RAMBanks = RAMSize = 0;
}
...
int WsCreateFromMemory(const uint8_t *romData, size_t romSize)
{
    ...
    /* Direct ROM XIP mapping.  Initialize both maps because
     * WsAllocateBuffers() only reserves zeroed pointer tables; a cartridge
     * without RAM must still map all writes to the open-bus dummy page. */
    s_rom_data = romData;
    s_cart_ram = NULL;
    for (i = 0; i < 256; ++i) {
        ROMMap[i] = MemDummy;
