...
/* The TLCS core addresses external RAM as 0x4000..0x17fff.  The old 32 KiB
 * allocation covered only part of this window and overwrote adjacent
 * emulator state.  Include the legacy state-save area at 0x20000 too. */
#define NGP_MAINRAM_SIZE 0x21000u
...
void ngp_mem_init(void)
{
    ...
    if (!mainram || !cpurom) {
        if (!ngp_mem_alloc_init(NGP_MAINRAM_SIZE, 0x10000)) { for(;;){} }
		cz80_bind_memory();
    }
