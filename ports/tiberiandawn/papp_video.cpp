// Video backend for the Tiberian Dawn PAPP (replaces common/video_sdl2.cpp).
//
...
// canvas every PAPP had before. Tiberian Dawn's UI is laid out for 640x400 (the
...
// Largest first; the same sizes as "canvas" in apps/psram_tiberiandawn/papp.json.
static const struct
{
    int w, h;
...
static void log_rate(long long convert_us, long long flush_us)
{
    ...
    if (now - window >= 60000000) {
        ...
        papp_svc->log_printf("TD: %d fps shown, %d fps from the game (%d dropped); per frame: copy %lld us, "
...
static void papp_present(const unsigned char* pixels, int w, int h)
{
    if (s_presenter == nullptr && s_presenter_done) {
        ...
        if (papp_svc->task_create(presenter_task, "td_present", 8 * 1024, nullptr, 3, &s_presenter, 1) != 0) {
            s_presenter = nullptr;
            papp_svc->log_printf("TD: no presenter task, presenting on the game task\n");
        }
    }
    s_offered++;
...
static void choose_canvas(int w, int h)
{
    ...
    papp_svc->log_printf("TD: offered a %dx%d canvas, using %dx%d\n", offer_w, offer_h, s_canvas_w, s_canvas_h);
}
...
{
    papp_svc->log_printf("TD: video mode %dx%d %d bpp\n", w, h, bits_per_pixel);
    s_mode_w = w;
    s_mode_h = h;
    papp_mouse_x = w / 2.0f;
    papp_mouse_y = h / 2.0f;
