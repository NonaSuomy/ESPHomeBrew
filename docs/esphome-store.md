...
- **Uninstall** asks first, in a dialog over the page. It removes `<install_dir>/<app>.papp` and its listing (`<app>.json`), so the app shows as not installed again. When the store has downloaded data for the app, a check box (off by default) also deletes that data. Only files the store itself downloaded are deleted: the loader records each one, with its size, in `<install_dir>/<app>.installed.json` as it downloads it (for Install and for Stream), and deletes a file only if it is still that size. Saves, settings (`settings.json`, including the Screen setting), files you copied yourself and the `requires` files stay. Folders the deleted files leave empty go too, except top-level ones such as `/sd/roms`. Uninstall is refused while an app is loading or running, or during an install. From lambdas: `id(papp_runtime)->uninstall_app("psram_doom", true)`.
