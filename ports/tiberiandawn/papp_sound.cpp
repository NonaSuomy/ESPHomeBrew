// Sound backend for the Tiberian Dawn PAPP (replaces common/soundio_openal.cpp).
//
...
bool SoundImp_Init(int bits_per_sample, bool stereo, int rate, bool reverse_channels)
{
    ...
    if (papp_svc->task_create(mixer_task, "td_mixer", 16 * 1024, nullptr, 6, &s_mixer, 1) != 0) {
        s_mixer = nullptr;
        s_mixer_done = true;
        papp_svc->log_printf("TD: could not start the sound mixer\n");
        return false;
    }
    papp_svc->log_printf("TD: sound %d Hz\n", s_out_rate);
    return true;
}
