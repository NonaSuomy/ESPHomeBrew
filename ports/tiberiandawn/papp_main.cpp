// Tiberian Dawn PAPP entry point.
//
...
int main(int argc, char **argv);  // tiberiandawn/startup.cpp
...
extern "C" void papp_quit(int code)
{
    ...
    papp_svc->log_printf("TD: quit(%d) before the game started\n", code);
    for (;;) {
...
static void game_task(void *)
{
    ...
    if (setjmp(s_exit_jmp) == 0) {
        ...
        papp_svc->log_printf("TD: running %u global constructors\n", ctors);
        for (init_fn* fn = __init_array_start; fn < __init_array_end; ++fn) {
            (*fn)();
        }
        papp_svc->log_printf("TD: constructors done, entering main()\n");
        s_exit_code = main(1, argv);
    }
    s_game_running = false;
    papp_svc->log_printf("TD: game ended (%d)\n", s_exit_code);
    // The loader does not reclaim an app's files or heap: hand everything back.
    // The mixer and presenter tasks read the game's buffers, so they stop first.
    papp_sound_shutdown();
    papp_video_shutdown();
...
{
    papp_svc = svc;
    svc->log_printf("TD: Vanilla Conquer Tiberian Dawn PAPP starting\n");
    ...
    if (svc->task_create(game_task, "tiberiandawn", 512 * 1024, nullptr, 5, &handle, 0) != 0) {
        svc->log_printf("TD: could not create the game task\n");
        return -1;
    }
    while (!s_game_done) {
        svc->delay_ms(50);
