...
#ifdef NEOGEO_PAPP
extern void neogeo_papp_log(const char *fmt, ...);
#define ZIP_DIAG(...) neogeo_papp_log(__VA_ARGS__)
#else
#define ZIP_DIAG(...) do { } while (0)
#endif
...
static int search_sig32(PKZIP *zf, uint8_t sig[4]) {
	...
	if (!zf || !zf->file)
		return -1;
	//pos=ftell(zf->file);
	/* PAPP FILE handles are owned by the launcher, so app-side feof() may
	 * inspect a different libc FILE state.  Use the wrapped fread() result as
	 * the end-of-file signal instead. */
	while (fread(&a, sizeof(char), 1, zf->file) == 1) {
		tr++;
		//printf("Search sig %d\n",a);
		if (sig[i] == a) {
			//printf("%02x %d\n",a,t);
...
static int search_central_dir(PKZIP *zf) {
	...
	ZIP_DIAG("NEO GEO DIAG: zip size via ftell=%ld\n", ftell(zf->file));
	zf->cde_offset = 0;
	...
	if (zf->cde_offset == -1) {
		ZIP_DIAG("NEO GEO DIAG: end-of-central-dir signature not found\n");
		printf("Couldn't find central dir, Corrupted zip\n");
		return -1;
	}
	ZIP_DIAG("NEO GEO DIAG: end record offset=%d\n", zf->cde_offset);
	...
	freadvle(zf->file, "22 22 44", &nbdsk, 0, 0, &zf->nb_item, &zf->cd_size,
			&zf->cd_offset);
	ZIP_DIAG("NEO GEO DIAG: entries=%u cd_size=%u cd_offset=%u\n",
			(unsigned)zf->nb_item, (unsigned)zf->cd_size,
			(unsigned)zf->cd_offset);
	if (nbdsk != 0) {
	...
	int cd_sig_offset = search_sig32(zf, sig_cd);
	ZIP_DIAG("NEO GEO DIAG: central-directory signature offset=%d expected=%u\n",
			cd_sig_offset, (unsigned)zf->cd_offset);
	if (cd_sig_offset != zf->cd_offset) {
...
//printf("CWR %s\n",get_current_dir_name());
	...
	ZIP_DIAG("NEO GEO DIAG: fopen(%s)=%p\n", file, (void *)zf->file);
	if (zf->file == NULL) {
