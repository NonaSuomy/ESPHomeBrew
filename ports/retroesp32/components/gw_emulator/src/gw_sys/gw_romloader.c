...
bool gw_romloader()
{
   ...
   /* A malformed or unsupported SD image is a normal PAPP load error.  Do
    * not assert here: aborting would reset the launcher instead of returning
    * cleanly to its library screen. */
   if (!rom_status)
      return false;
