...
static INLINE void tlcsFastMemWriteB(unsigned int addr,unsigned char data)
{
    ...
    unsigned int masked = addr & 0xFFFFFF;
    if (masked < 0x00004000 ||
        (size_t)(masked - 0x00004000) >= g_mainram_size)
        return;
	mainram[masked - 0x00004000] = data;
}
...
static INLINE void tlcsFastMemWriteW(unsigned int addr, unsigned short data)
{
    ...
    unsigned int masked = addr & 0xFFFFFF;
    if (masked < 0x00004000 ||
        (size_t)(masked - 0x00004000) > g_mainram_size - 2)
        return;
    unsigned char*ram = mainram + (masked - 0x00004000);
...
static INLINE void tlcsFastMemWriteL(unsigned int addr, unsigned int data)
{
    ...
    unsigned int masked = addr & 0xFFFFFF;
    if (masked < 0x00004000 ||
        (size_t)(masked - 0x00004000) > g_mainram_size - 4)
        return;
    unsigned char*ram = mainram + (masked - 0x00004000);
