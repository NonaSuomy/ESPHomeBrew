...
#define NEO_DIAG(...) neogeo_papp_log(__VA_ARGS__)
#else
#define NEO_DIAG(...) do { } while (0)
#endif
...
ROM_DEF *res_load_drv(char *name) {
	...
	NEO_DIAG("NEO GEO DIAG: datafile=%s stat=%d size=%ld mode=0x%lx\n",
			gngeo_dat ? gngeo_dat : "(null)", stat(gngeo_dat, &sb),
			(long)sb.st_size, (unsigned long)sb.st_mode);

	/* Open the rom driver def */
	pz = gn_open_zip(gngeo_dat);
	NEO_DIAG("NEO GEO DIAG: gn_open_zip=%p\n", (void *)pz);
	if (pz == NULL) {
		free(drv);
	...
	NEO_DIAG("NEO GEO DIAG: looking for %s\n", drvfname);

	if ((z=gn_unzip_fopen(pz,drvfname,0x0)) == NULL) {
		NEO_DIAG("NEO GEO DIAG: driver open failed\n");
		free(drv);
		gn_close_zip(pz);
		//fprintf(stderr, "Can't open rom driver for %s %s\n", name,drvfname);
		return NULL;
	}
	NEO_DIAG("NEO GEO DIAG: driver open OK\n");
